...
/**
 * IP camera device.
 * Modded allowing TimeOut on http connection and change isOnline function for better handling.
 * @author Bartosz Firyn (sarxos), modded by Pier Riccardo Monzo
 */
public class IpCamMyDevice implements WebcamDevice, FPSSource, BufferAccess {

	/**
	 * Logger.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(IpCamMyDevice.class);
	...
	public IpCamMyDevice(String name, String url, IpCamMode mode) throws MalformedURLException {
		this(name, new URL(url), mode, null);
	}

	public IpCamMyDevice(String name, URL url, IpCamMode mode) {
		this(name, url, mode, null);
	}

	public IpCamMyDevice(String name, String url, IpCamMode mode, IpCamAuth auth) throws MalformedURLException {
		this(name, new URL(url), mode, auth);
	}

	public IpCamMyDevice(String name, URL url, IpCamMode mode, IpCamAuth auth) {
	...
	private HttpClient createClient() {
		/*
		 * HO DOVUTO COPIARE DUE CLASSI DAL PACCHETTO, CAMBIARE NOME E GESTIRE
		 * I VARI IMPORT, PER AGGIUNGERE QUESTE TRE CA**O DI RIGHE
		 * PERCHE' L'EMERITA T.D.M. NON HA PENSATO DI AGGIUNGERE UN PARAMETRO
		 * PER GESTIRE IL TIMEOUT (va in eccezione dopo 20 fo**uti secondi,
		 * bloccando nel frattempo tutta l'applicazione).
		 * 
		 * ANZI, L'AVEVA AGGIUNTO (come da commento su #460) MA POI E' MAGICAMENTE
		 * SPARITO.
		 * 
		 * CA**O, CA**O, CA**O, CA**O, CA**O, CA**O, CA**O, CA**O, CA**O, CA**O !!!
		 * 
		 * SARXOS, VAI A FARTI FO**ERE!
		 */
